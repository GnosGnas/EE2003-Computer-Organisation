...
#define MULT_RES_LOWER (MULT_BASE + 12)
#define MULT_RES_UPPER (MULT_BASE + 4)
...
int Mult_GetResult_upperword(void);
...
int Mult_GetResult(void)
{
	volatile int *p = (int *)MULT_RES_LOWER;
	return (*p);
}

/// Added another function to include upperword
int Mult_GetResult_upperword(void)
{
	volatile int *p = (int *)MULT_RES_UPPER;
	return (*p);
}
////
void hello(void)
{
	...
	//int xu = Mult_GetResult_upperword();
	//print_dec(xu);
	//print_str("*1024+");
	print_dec(x);
	print_str("\n");
